...
- **堆与栈**  
  - 栈：由系统自动分配的内存区域，由系统自动释放，存放函数的参数值，局部变量的值等。其 
  操作方式类似于数据结构中的栈。例如，在函数中声明一个局部变量 int b; 系统自动在栈中为b开辟空间  
  - 堆：需要程序员自己申请的内存区域，并指明大小，并且需要主动进行释放。若程序员不释放，程序结束时可能由OS回 
  收 例如: 在c中malloc函数，如p1 = (char*)malloc(10);在C++中用new运算符如p2 = new char[10]

作者：川少叶
链接：https://www.jianshu.com/p/f347af8919cb
来源：简书
