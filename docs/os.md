...
- **进程与线程**  
  ...
  - **线程共享哪些进程资源**  
    地址空间，全局变量，堆（全局堆和局部堆），打开文件集，子进程，定时器，全局堆就是所有没有分配的空间，局部堆就是用户分配的空间  
  - **线程独占的资源**  
    程序计数器：用来记录接着要执行哪一条指令  
    寄存器：用来保存线程当前的工作变量  
    堆栈：用来记录执行历史，其中每一帧供各个被调用但是没有返回的过程使用，在该帧中存放了相应过程的局部变量以及过程调用完成之后使用的返回地址  
    ![img](./pic/thread.png)  
...
- **进程调度算法**  
  - **何时调度** 1. 创建新进程后，决定父进程or子进程运行；2. 进程退出时必须做出调度决策；3. 进程阻塞在I/Ohe信号量上or由于其他原因阻塞时；4. 当I/O中断发生时
  - **批处理系统** 批处理系统没有太多的用户操作，在该系统中，调度算法目标是保证吞吐量和周转时间（从提交到终止的时间）（吞吐量，周转时间，CPU利用率）  
  ...
  - **交互式系统** 交互式系统有大量的用户交互操作，在该系统中调度算法的目标是快速地进行响应（响应时间，均衡性）  
    - 时间片轮转  
    ...
    - 彩票调度  
      为进程提供各种系统资源的彩票，当需要做出决策的时候，随机选择，获得资源。（每秒50次的彩票，每人获得20msCPU）
  - **实时系统**（满足截止时间，可预测性）  
...
- **线程调度**  
  要分用户级线程or内核级线程  
  用户级线程：内核不值线程存在，so分配给A进程时间片，A中的线程调度决定哪个线程运行。进程内无时钟中断，soA1线程可以按意愿运行希望的时长。若线程占用完时间片，则进程调度，A1等待下次继续运行。若较短，则线程轮转调度和优先级调度，无时钟中断是其局限。  
  内核级线程：内核选择一个线程运行，无需考虑属于哪个进程。对被选择的线程赋予一个时间片，如果超过时间片，就会刮起该线程。  
  两者差异在性能：用户级的线程切换只需要少量机器指令，和内核级需要完整的上下文切换，修改内存映像，使高速缓存失效，另一方面，使用内核级时，一旦线程阻塞在I/O上，不需要像用户级那样将整个进程挂起  
- **进程同步**  
  - **临界区**
...
- **物理地址暴露给进程的问题** 
  1. 如果用户程序可以寻址内存的每个字节，就可以很容易破坏操作系统 2. 想同时运行多个程序很困难，一个在工作，另一个按下鼠标才开始
...
- **空闲内存管理**  
  - 使用位图的存储管理（0表示空闲，1占用或者相反）  
  - 链表：H表示空闲区，P表示进程。为创建的进程分配内存，
    1. 首次适配算法   
    2. 下次适配算法（每次找到合适的空闲区都记录当时的位置，方便下次寻找时从上次结束地方开始搜索）   
    3. 最佳适配算法（找出能够容纳进程的最小空闲区）  
    4. 最差适配算法（总分配最大的空闲区）   
    四种算法的优化：为进程和空闲区维护各自独立的链表，代价时增加复杂度和内存释放的速度变慢，因为需要将一个回收的段从进程链表中删除然后插入到空闲区链表  
    若空闲区按照大小排序，首次和最佳一样快，下次无意义。  
    另一种快速适配算法，为那些常用大小的空闲区维护单独的链表，例如4k，8k，12k... 像21k这样的空闲区既可以放在20k的链表中，也可以专为特别大小的空闲链表中  
  ![img](./pic/freeram.png)  
  ![img](./pic/freeram2.png)  
...
- **内存超载**  
  - 交换：把一个进程完整调入内存，使其运行一段时间，然后存回磁盘，空闲进程主要在磁盘  
  - 虚拟内存

- **虚拟内存**  
  虚拟内存的目的是为了让物理内存扩充成更大的逻辑内存，从而让程序获得更多的可用内存。  
  为了更好的管理内存，操作系统将内存抽象成地址空间。每个程序拥有自己的地址空间，这个地址空间被分割成多个块，每一块称为一页，每一页有连续的地址范围。这些页被映射到物理内存，但不需要映射到连续的物理内存，也不需要所有页都必须在物理内存中。当程序引用到不在物理内存中的页时，由硬件执行必要的映射，将缺失的部分装入物理内存并重新执行失败的指令。  
...
- **虚拟地址空间**  
  ...
  虚拟地址空间按照固定大小划分为被成为页面（page）的若干单元，在物理内存中对应的单元成为页框（page frame），页面和页框的大小通常一样（here 4k，actually512byte~1GB)，对应于64kb的虚拟地址空间和32KB的物理内存，可得到16个虚拟页面和8个页框
...
- **内存碎片**  
  ...
  - 内部碎片：内部碎片是由于采用固定大小的内存分区，当一个进程不能完全使用分给它的固定内存区域时就产生了内部碎片，通常内部碎片难以完全避免  
...
- **缺页中断**  
  - 硬件陷入内核，在堆栈中保存程序计数器（pc），大多数机器将当前指令的各种状态信息保存在特殊的CPU寄存器中（SR0， SR2？）  
  - 启动汇编代码程序保存通用寄存器和其他信息，以免被操作系统破坏，这个程序将操作系统作为一个函数来调用  
  - 当操作系统发现一个缺页中断时，尝试发现需要哪个虚拟页面，通常一个硬件寄存器包含这一信息，如果没有的话，操作系统必须检索程序计数器，取出指令，用软件分析指令，看看缺页中断时的操作  
  - 一旦知道了发生缺页中断的虚拟地址，操作系统检查这个地址是否有效，并检查存取与保护是否一致，若不一致，向进程发出一个信号or杀掉进程。如果地址有效且保护位没有错误发生，系统检测是否有空闲页框。如果没有空闲页框，执行页面置换算法  
  - 如果选择的页框脏了（已修改），安排该页写回磁盘，并发生一次上下文切换，挂起产生缺页中断的进程，让其他进程运行直至磁盘传输结束。无论如何，该页框标记为忙，以免因为其他原因而被其他进程占用  
  - 一旦页框干净后（无论是立刻还说在写回磁盘后），操作系统查找所需页面在磁盘上的地址，通过磁盘操作将其装入。在该页被装入时，产生缺页中断的程序仍然被挂起，如果有其他可运行的用户进程，则可进行运行  
  - 当磁盘中断发生时，表明该页已经装入，页表已经更新可以反映它的位置，页框也被标记为正常状态  
  - 回复发生缺页中断指令以前的状态，程序计数器重新指向这条指令  
  - 调度引发缺页中断的进程，操作系统返回调用他的汇编代码  
  - 该例程恢复寄存器和其他寄存器的状态信息，返回到用户空间继续执行
